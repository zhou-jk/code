            ...
            if(t==2)
            {
                if(G[id].edge[i].val!=0)
                {
                    if(dis[2][v]>dis[2][u]+G[id].edge[i].val)
                    {
                        ...
                        q.push(make_pair(dis[2][v],2*n+v));
                    }
                }
            }
            if(t==1)
...
int main()
{
    ...
    Dijkstra(1,U);
    long long ans1=min(min(dis[0][V],dis[1][V]),dis[2][V]);
    Dijkstra(1,V);
    long long ans2=min(min(dis[0][U],dis[1][U]),dis[2][U]);
