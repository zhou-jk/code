...
void build(int n)
{
	...
	for (int i=1; i<=log[n-1]; i++)
		for (int j=0; j<=n-bin[i]; j++) st[i][j]=max(st[i-1][j],st[i-1][j+bin[i-1]]);
}
...
int main()
{
	...
	for (int i=0; i<m; i++)
	{
		...
		//printf("%d %d\n",l,r);
		if (d<=c) puts("false");
		else if (r<0||l>=n||(a[l]>c&&a[r]<d)) puts("maybe");
		else
		{
			if (a[l]==c&&a[r]==d)
			{
				if (b[r]>b[l]||query(l+1,r-1)>=b[r]) puts("false");
				else if (r-l==d-c) puts("true");
				else puts("maybe");
			}
			else if (a[l]==c)
			{
				if (query(l+1,r)>=b[l]) puts("false");
				else puts("maybe");
			}
			else if (a[r]==d)
			{
				if (query(l,r-1)>=b[r]) puts("false");
				else puts("maybe");
			}
		}
	}
