...
int n;
Point p[N];
bool check(const int mid,const vector<int> &pos)
{
    for(int i=0;i<(int)pos.size();i++)
        if(pos[i]!=mid)
        {
            Line l=Line(p[mid],p[pos[i]]);
            for(int j=i+1;j<(int)pos.size();j++)
                if(pos[j]!=mid)
                {
                    if(l.direction(p[pos[j]])==ONLINE_FRONT||l.direction(p[pos[j]])==ON_SEGMENT) return false;
                }
        }
    return true;
}
void solve()
{
    ...
    bool flag=false;
    for(int i=3;i<=4;i++)
        if(l.direction(p[i])!=ONLINE_FRONT&&l.direction(p[i])!=ONLINE_BACK&&l.direction(p[i])!=ON_SEGMENT)
        {
            flag=true;
            break;
        }
    for(int i=5;i<=n;i++)
        if(flag||(l.direction(p[i])!=ONLINE_FRONT&&l.direction(p[i])!=ONLINE_BACK&&l.direction(p[i])!=ON_SEGMENT))
        {
            vector<int>pos={1,2,3,4,i};
            for(int t=0;t<5;t++)
            {
                if(check(pos[t],pos))
                {
