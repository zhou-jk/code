...
vector<int> half_plane_intersection(const vector<pair<Line,int>> &l,double x1=-INF,double y1=-INF,double x2=INF,double y2=INF)
{
    vector<pair<double,pair<Line,int>>>f;
    for(int i=0;i<(int)l.size();i++)
        f.emplace_back((l[i].first.b-l[i].first.a).angle(),l[i]);
    f.emplace_back(0,make_pair(Line(Point(x1,y1),Point(x2,y1)),-1));
    f.emplace_back(PI/2,make_pair(Line(Point(x2,y1),Point(x2,y2)),-1));
    f.emplace_back(PI,make_pair(Line(Point(x2,y2),Point(x1,y2)),-1));
    f.emplace_back(-PI/2,make_pair(Line(Point(x1,y2),Point(x1,y1)),-1));
    int n=f.size();
    sort(f.begin(),f.end(),[](const pair<double,pair<Line,int>> &a,const pair<double,pair<Line,int>> &b)
    {
        return !equal(a.first,b.first)?a.first<b.first:a.second.first.direction(b.second.first.a)==CLOCKWISE;
    });
    vector<pair<Line,int>>Ql(n);
    vector<Point>Qp(n);
    vector<int> res;
    int head=0,tail=-1;
    Ql[++tail]=f[0].second;
    for(int i=1;i<n;i++)
        if(!equal(f[i].first,f[i-1].first))
        {
            while(head<tail&&f[i].second.first.direction(Qp[tail-1])==CLOCKWISE) tail--;
            while(head<tail&&f[i].second.first.direction(Qp[head])==CLOCKWISE) head++;
            Ql[++tail]=f[i].second;
            if(head<tail)
            {
                vector<Point> tmp=cross_point(Ql[tail].first,Ql[tail-1].first);
                if(!tmp.empty()) Qp[tail-1]=tmp[0];
                else return res;
            }
        }
    while(head<tail&&Ql[head].first.direction(Qp[tail-1])==CLOCKWISE) tail--;
    while(head<tail&&Ql[tail].first.direction(Qp[head])==CLOCKWISE) head++;
    if(tail-head+1<=2) return res;
    vector<Point> tmp=cross_point(Ql[tail].first,Ql[head].first);
    if(tmp.empty()) return res;
    for(int i=head;i<=tail;i++)
        if(Ql[i].second!=-1) res.push_back(Ql[i].second);
...
void solve()
{
    ...
    for(int i=0;i<n;i++)
        cin>>p[i];
    vector<pair<Line,int>> l(n-1);
    for(int i=1;i<n;i++)
        l[i-1]=make_pair(Line(p[0],p[i]).perpendicular_bisector(),i);
    vector<int>res=::half_plane_intersection(l);
    int m=res.size();
    cout<<m;
    for(int u:res)
        cout<<" "<<u;
