...
void solve()
{
    ...
    for(int r=1;r<=n;r++)
    {
        ...
        {
            int len=r-l+1;
            ans[id]=((long long)len*(len+1)*(len+2)/6+Tl.getsum(l)*l+Tr.getsum(l)*r+(long long)l*r*Tlr.getsum(l)+T.getsum(l))%MOD;
            ans[id]=(ans[id]+MOD)%MOD; 
            ans[id]=(long long)ans[id]*getinv((long long)len*(len+1)/2%MOD)%MOD;
        }
    }
    for(int i=1;i<=q;i++)
        cout<<ans[i]<<"\n";
