...
int main()
{
    ...
    for(int i=1;i<=m;i++)
    {
        if(pos[i].empty())
        {
            T.modify(1,1,k,make_pair(x[i],y[i]));
            continue;
        }
        if(1<=pos[i].front()-1) T.modify(1,1,pos[i].front()-1,make_pair(x[i],y[i]));
        for(int j=1;j<(int)pos[i].size();j++)
            if(pos[i][j-1]+1<=pos[i][j]-1) T.modify(1,pos[i][j-1]+1,pos[i][j]-1,make_pair(x[i],y[i]));
        if(pos[i].back()+1<=k) T.modify(1,pos[i].back()+1,k,make_pair(x[i],y[i]));
    }
    for(int i=1;i<=n;i++)
