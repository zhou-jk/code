...
int query(int u,int v,int l,int r,int k)
{
    if(l==r) return b[l];
    int mid=(l+r)/2;
    if(tree[tree[v].lc].sum-tree[tree[u].lc].sum>k) return query(tree[u].lc,tree[v].lc,l,mid,k);
    if(tree[tree[v].rc].sum-tree[tree[u].rc].sum>k) return query(tree[u].rc,tree[v].rc,mid+1,r,k);
    return 0;
}
int main()
{
